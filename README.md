...
Drop the live patch version into any README or project page:

```markdown
![SpiritVale latest patch](https://img.shields.io/endpoint?url=https://spiritvale.tama.sh/shields/version.json)
```

Endpoint: `/shields/version.json` (alias for `/badge/latest.json`) — regenerated by `make badge` on every patch release.
