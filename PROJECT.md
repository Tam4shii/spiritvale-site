...
**Last `make check` run**: 2026-06-07T01:20Z (idle-loop Forge 2026-06-07 run#8) — ✅ exit 0 (all 8 artifacts valid; baseline OK at 0.18.0)
...
**Last Steam check**: 2026-06-07T01:19:57Z (idle-loop Forge 2026-06-07 run#8) — ✅ live Steam API poll (HTTP 200); items_found=10; new_draft=false. **🚨 ANNOUNCEMENT DRAFT STILL PENDING**: `patches/drafts/announcement-spiritvale-official-date-release.json` — seen_count=6 (first_seen: 2026-06-05T20:31Z, last_seen: 2026-06-07T01:19Z). Stale-alert suppressed (already alerted < 12h ago). Awaiting boss decision on routing (PR #1).
**🚨🚨 DEADLINE TODAY/TOMORROW**: Playtest ends **2026-06-08 (TOMORROW)**. Demo launches **2026-06-12**. Early Access **2026-07-15** ~$15 USD. `make check-deadlines` → CRITICAL (days_until=1).
**run#8 cleanup**: Removed stale untracked `patches/draft-spiritvale-official-date-release.json` (root-level duplicate missing `"type": "announcement"`; canonical version in `patches/drafts/`).
