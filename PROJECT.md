...
**Next idle-loop action**: BOSS ACTION NEEDED — review `patches/draft-spiritvale-official-date-release.json`. Decide: (a) publish as `patches/news/spiritvale-launch-dates.json` + news page, (b) `git rm` + commit (archive it), (c) leave untracked (ignore). After decision, run `make check-steam` to sync baseline.
