...
- **NEXT**: Check Steam API for new patch (fetch `https://api.steampowered.com/ISteamNews/GetNewsForApp/v2/?appid=3918510&count=5&format=json`, compare GIDs against `patches/index.json`; if new patch found, parse and add as new `patches/vX.Y.Z.json`, update index/feed/sitemap/llms.txt/CHANGELOG, regenerate `og/vX.Y.Z.png` via `make og`)
